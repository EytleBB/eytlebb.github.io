...
class MuseumGuestbookHTTPTests(unittest.TestCase):
    ...
    def test_static_preview_denies_internal_hidden_traversal_and_symlinks(self):
        ...
        (self.public / "models").mkdir()
        (self.public / "models/knife.glb").write_bytes(b"glTF")
        (self.public / "models/private.txt").write_text("private")
        self.assertEqual(self.client.request("GET", "/models/knife.glb", full_path=True)[0], 200)
        self.assertEqual(self.client.request("GET", "/models/private.txt", full_path=True)[0], 404)
